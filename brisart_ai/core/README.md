...
2. Builds the allowed source types from the current settings — `web` is **always** included (previously-indexed pages stay searchable regardless of the Local Files toggle); `file` and `note` are added per-toggle.
...
Uses `check_same_thread=False` because web research runs on a background thread while the connection is created on the main thread; the app's `_busy` flag already serializes access, so no extra locking is needed. `recent_topics(limit=6)` returns the most-recent, de-duplicated topics newest-first.
...
`load()` creates the file with defaults if it doesn't exist, and silently falls back to defaults on a corrupt/unreadable file rather than raising — a broken settings file can never crash startup. Only known boolean keys are accepted on load; a stale key from an older build is ignored, not re-persisted. `SETTING_ALIASES` maps short typed keys (`"web"`, `"local"`, `"notes"`) to their canonical setting name. JSON is read and written via the native `brisart_json`.
