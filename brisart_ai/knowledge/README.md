...
Two tables: `sources` (type, location, title, full text, content hash, size, extension, timestamp) and `terms` (flat term-frequency rows keyed by `(term, source_id)`, read directly by `ranker.py`). `DEFAULT_DB` is anchored to the project root, so the database always lands at `<project root>/brisart_ai_index.sqlite3` regardless of the working directory the app was launched from.

`add_source()`'s `source_key` is a stable hash (via the native `brisart_hash`) of `source_type + location`, so re-adding a file or re-crawling a URL **fully replaces** both the `sources` row and every `terms` row for it. Raises `ValueError` for a missing type/location, but returns `False` for empty text.
...
`ranker.py` layers five more signals on top: coverage (rewards matching more distinct meaningful query terms), title match (damped for generic instructional verbs like "explain"), generic-concept-title penalty (a document titled nothing but a bare abstract concept word is demoted, unconditionally, even without a specific detected intent), phrase match (the literal query text as a contiguous phrase earns a flat multiplier), and intent (`intent.py` supplies the "why does this mention the query's words" signal).
...
Structure layered on top of the raw index: research collections, local notes, lightweight capitalized-phrase entity extraction (a regex heuristic, not machine learning), source-to-entity links, a topic timeline, and a vault summary report.

Notes are mirrored into the main `sources` table (`source_type="note"`) so they get ranked by the exact same model as files and web pages. `reindex_missing_notes()` runs once at service startup to bring notes saved before this mirroring existed up to date; it's idempotent, so re-running it costs nothing on an already-current vault. Collections, entity extraction, and the timeline view remain implemented here but are not currently exposed through the desktop UI (`ui/service.py` only calls `add_note`, `list_notes`, `reindex_missing_notes`, and `search_notes`).
